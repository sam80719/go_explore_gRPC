...
.DEFAULT_GOAL := help
.PHONY: greeting blog calculator help
project := greeting calculator blog

all: $(project) ## Generate Pbs and build

greeting: $@ ## Generate Pbs and build for greeting
...
clean: clean_greeting clean_calculator clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_greeting: ## Clean generated files for greeting
	${RM_F_CMD} greeting/${PROTO_DIR}/*.pb.go
